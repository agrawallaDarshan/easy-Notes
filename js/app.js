...
function shownotes() {
    ...
    notesArray.forEach(element => {
        cardhtml += `<div class="card my-3 mx-3 noteCard" style="width: 18rem; border: 1px solid black;">
        <div class="card-body">
          <h5 class="card-title">${element[0]}</h5>
          <p class="card-text">${element[1]}</p>
          <button id = "${notesArray.indexOf(element)}" onclick="deleteNote(this.id)" class="btn btn-primary">Delete note</button>
          <button onclick = "markorunmark(${notesArray.indexOf(element)})" class="btn btn-primary my-2">Mark as Imp</button>
        </div>
      </div>`;
    });
    ...
    if (notesArray.length != 0) {
        ...
        notes.innerHTML = `Nothing to show. Please add notes to display and save your notes`;
    }
}
